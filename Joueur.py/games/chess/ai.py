...
import games.chess.pranith.chessFunctions
...
class AI(BaseAI):
    ...
    def get_name(self) -> str:
        ...
        return "Naruto" # REPLACE THIS WITH YOUR TEAM NAME
    ...
    def make_move(self) -> str:
        ...
        board = games.chess.pranith.chessFunctions.boardGenerator( fen_string )
        # printBoard(board)
        turn = fen_string.split()[1]
        if turn == 'w':
            color = games.chess.pranith.chessFunctions.WHITE
        else:
            color = games.chess.pranith.chessFunctions.BLACK

        # KILLER MOVES IMPLEMENTED

        t = self.player.time_remaining
        h = len(self.game.history)
        self.killer_moves = games.chess.pranith.chessFunctions.timeLimitedIDDLMinimax2( board, color, t, h/2, self.killer_moves )
        self.killer_moves.reverse()
        
        print('killer moves', list(map(games.chess.pranith.chessFunctions.getUCI, self.killer_moves)))
        best_move = games.chess.pranith.chessFunctions.getUCI( self.killer_moves[0] )
